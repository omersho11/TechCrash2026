...
struct NeuralNetwork {
    float w1[4][4]; // input to hidden weights
    float b1[4];    // hidden layer biases
    float w2[4];    // hidden to output weights
...
float random_float(float min_val, float max_val) {
    float r = (float)random(10000) / 10000.0;
...
void updateDifficultySettings() {
    ...
    // Spacing between obstacles decreases at higher difficulties
...
void nextGeneration() {
    // Sort birds by fitness (descending order) - Bubble sort
    ...
    // Capture overall stats
    if ((int)population[0].score > bestScore) {
        bestScore = population[0].score;
    }
    if ((int)population[0].fitness > bestFitness) {
        bestFitness = population[0].fitness;
    }

    // Keep top 6 (Elites) and clone/mutate them for the rest
    // Index 0..5 are elites.
    for (int i = 6; i < POPULATION_SIZE; i++) {
        int parentIdx = i % 6; // Select from top 6 parents
        population[i].brain = population[parentIdx].brain;
        mutateBrain(population[i].brain, 0.15); // 15% mutation rate
    }

    // Reset states for all birds
...
void loop() {
    ...
    // Find next obstacle details
    int nextPipeIdx = 0;
    float nextPipeX = obstacleX[0];
    float nextPipeGapY = obstacleGapY[0];

    // Determine which pipe is closest ahead of the bird (X = OLED_WIDTH/4.0)
    ...
    // Update birds physics and brain decision
    aliveCount = 0;
    int bestBirdIdx = -1;
    ...
    for (int i = 0; i < POPULATION_SIZE; i++) {
        if (population[i].alive) {
            ...
            // Decides whether to flap using neural network
            ...
            if (population[i].alive) {
                if (population[i].fitness > maxFitnessSoFar) {
    ...
    for (int p = 0; p < 2; p++) {
        ...
        if (obstacleX[p] < -OBSTACLE_WIDTH) {
            int other = 1 - p;
            obstacleX[p] = obstacleX[other] + pipeDistance;
            obstacleGapY[p] = 16 + (my_rand() % (GROUND_Y - 32));
            
            // Increment score for all alive birds
            for (int i = 0; i < POPULATION_SIZE; i++) {
                if (population[i].alive) {
                    population[i].score++;
                }
    ...
    if (showOnlyBest) {
        if (bestBirdIdx != -1) {
            display.drawBitmap(OLED_WIDTH / 4.0, (int)population[bestBirdIdx].y, bird_bmp, BIRD_WIDTH, BIRD_HEIGHT, SSD1306_WHITE);
        }
    } else {
        for (int i = 0; i < POPULATION_SIZE; i++) {
            if (population[i].alive) {
                // If it is the best bird, draw full bitmap, otherwise draw a small pixel/box to prevent overlapping screens
                if (i == bestBirdIdx) {
                    display.drawBitmap(OLED_WIDTH / 4.0, (int)population[i].y, bird_bmp, BIRD_WIDTH, BIRD_HEIGHT, SSD1306_WHITE);
                } else {
                    display.fillRect(OLED_WIDTH / 4.0 + 4, (int)population[i].y + 2, 3, 3, SSD1306_WHITE);
                }
            }
        }
    }
    ...
    display.printf("G:%d A:%d B:%d", generation, aliveCount, bestScore);
